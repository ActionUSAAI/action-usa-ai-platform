...
**Importante para implementación futura:** los formularios XFA son conocidos por ser propensos a error al rellenar solo el AcroForm de respaldo — algunos visores (incluido potencialmente el propio sistema de USCIS) pueden priorizar el paquete XFA original sobre lo escrito en el shadow AcroForm. Antes de construir el llenado automático real, validar con una prueba de relleno + apertura en Adobe Reader que los datos se reflejen correctamente, no asumir que escribir en el AcroForm es suficiente.
...
8. **Requisito estructural de O-1B: dos organizaciones de consulta simultáneas** (Labor Organization + Management Organization — bloques 11 y 12 del O/P Supplement), a diferencia de O-1A que solo requiere una (bloque 10). Esto NO fue verificado con evidencia real en esta sesión — fue una afirmación de una sesión de chat anterior sin confirmación en el JSON de campos reales. Pendiente de verificar contra el texto real de las instrucciones del formulario antes de asumirlo como cierto.
