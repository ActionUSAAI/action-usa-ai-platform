...
MR — Re-run (post-CR-CPS-43 F-01 correction). See CR-CPS-43 below for
the correction record. NEXT MTCS remains NOT ESTABLISHED; Production
remains HARD-DENIED.
...
| AUSCIS Intake Intelligence Layer — Coach / CV(optional) / A0 / Structured Profile / Prefill Engine / Automated Readiness (Final Exact Design CR-CPS-34, Implementation Authorization CR-CPS-35, Implementation CR-CPS-36, Reconciliation R-01/R-02 CR-CPS-37, Implementation Authorization Re-run CR-CPS-38, Implementation CR-CPS-39, Implementation MR CR-CPS-40, Corrections CR-CPS-41, Implementation MR Re-run CR-CPS-42, F-01 Security Correction CR-CPS-43, reconciled from CR-CPS-32/33) | AUSCIS | Stage 1 acquisition/discovery/structuring/completion of beneficiary case information via Coach-led conversational discovery + optional CV extraction, automated readiness, prior to A1 handoff | docs/AUSCIS_INTAKE_INTELLIGENCE_LAYER_FINAL_EXACT_DESIGN.md; docs/AUSCIS_INTAKE_INTELLIGENCE_LAYER_RECONCILIATION_R01_R02.md; AUCIS_EVIDENCE_ITEM_CONTRACT_V2.md | FROZEN | FROZEN (design), IMPLEMENTED (TEST only, D-1/D-2/D-3/F-01 all corrected) | IMPLEMENTATION MR RE-RUN PENDING — F-01 CORRECTED, awaiting independent re-audit | INTEGRATED (TEST only, D-1/D-2/D-3/F-01 all corrected) | KEEP | **F-01 SECURITY CORRECTION — CORRECTED, TEST ONLY, PENDING IMPLEMENTATION MR RE-RUN (CR-CPS-43)** | Token expiry is 14 days in repository vs 15 days reported as current operation (non-blocking factual note, explicitly not altered per Project Owner instruction); future tenant review-policy override explicitly NOT ESTABLISHED | `AUSCIS Intake Intelligence Layer` — Implementation MR — Re-run (post-CR-CPS-43 F-01 correction) | NO |
...
└── AUSCIS
    ...
    │   │      Readiness — F-01 SECURITY CORRECTION — CORRECTED, TEST
    │   │      ONLY, PENDING IMPLEMENTATION MR RE-RUN (CR-CPS-43):
    │   │      isCvPathAuthorizedForInvitation() binds A0's filePath to
    │   │      the resolved invitation, fail-closed before any storage
    │   │      access · commit 53bcc54, 3 files · 77/77 PASS live
    │   │      (F01-CORR-01..10 net new; SEC-CORR-01..12 D-1/D-2/D-3
    │   │      regression confirmed intact) · token expiry 14 days
    │   │      (repo) vs 15 (reported ops), not altered · see Section Q
    │   │      for full CR-CPS-34..42 history · A1–A5/AKAE/AEPE
    │   │      untouched · no migration · UNNUMBERED · Production
    │   │      HARD-DENIED · next act: Implementation MR — Re-run
    │   │      (post-CR-CPS-43)
    │   ├── ○ Organization/Multi-Tenant
