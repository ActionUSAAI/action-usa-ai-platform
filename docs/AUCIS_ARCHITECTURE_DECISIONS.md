...
## ADR-011 — Evidence Lifecycle, Reassessment and Strategic Consumption

**Fecha:** 2026-09-10
**Estado:** Aprobada

**Contexto:** La arquitectura vigente (`AUCIS_EVIDENCE_ITEM_CONTRACT_V1.md`, `AUCIS_DOMAIN_INTERACTION_ARCHITECTURE.md`, `AUCIS_CRITERION_ASSESSMENT_CONTRACT_V1.md`, `AUCIS_CORE_DOMAIN_MODEL.md`) definía Evidence Item mediante un lifecycle lineal (`Reported → Documented → Verified → Used`), con Intake/A0 como owner exclusivo, un único `supporting_document_ref`, y una cascada de invalidación que marcaba automáticamente `stale` al Criterion Assessment y al Case Blueprint cuando un Evidence Item cambiaba — aunque la regeneración real nunca fue automática (Human-in-the-Loop ya vigente). Un MR arquitectónico focalizado (2026-09-10), contra los nueve documentos directamente afectados, confirmó que ninguna de las reglas a modificar viola una autoridad superior no superseder por un ADR explícito.

**Problema:** ¿Evidence Item debe seguir modelado como una unidad con lifecycle lineal, owner exclusivo (Intake), un solo documento de soporte, y staleness automática de Criterion Assessment/Blueprint al cambiar — o debe reestructurarse como una unidad probatoria factual + documental, propiedad del Case, capturable a través de múltiples canales gobernados, con condiciones de madurez independientes entre sí, sin invalidar automáticamente decisiones jurídicas ya versionadas?

**Alternativas consideradas:**
1. Mantener el lifecycle lineal y la cascada automática de marcado `stale` tal como estaban documentados.
2. Modelar Evidence Item como unidad factual + documental propiedad del Case, con Documentary Condition y Human Verification Condition como dimensiones independientes (no secuenciales), reassessment de A1 y regeneración de Blueprint siempre disparados por acción humana explícita, y selección estratégica como relación Blueprint-versión-específica en vez de un estado intrínseco de Evidence.

**Decisión:** Opción 2. El detalle normativo completo — 19 Decisiones (D-001–D-019) y 18 Invariantes (INV-EV-01–INV-EV-18) — se preserva íntegramente en el **Detailed Normative Decision Record** de este ADR: `docs/ADR-011_EVIDENCE_LIFECYCLE_REASSESSMENT_AND_STRATEGIC_CONSUMPTION.md`. Ese documento es parte integral de ADR-011 — no constituye un ADR independiente, no posee autoridad propia, no introduce una categoría nueva dentro del corpus `governance/level-*`, y deriva toda su autoridad exclusivamente de esta entrada.

**Justificación:** separar hecho probatorio, soporte documental, procesamiento (A2), verificación humana, evaluación de criterios (A1), decisión estratégica (A5), y ejecución (A3/A4) en responsabilidades distinguibles evita que un cambio incidental de Evidence dispare, incluso solo como señal automática, la invalidación de decisiones jurídicas ya versionadas y aprobadas — reforzando, no debilitando, Human-in-the-Loop. La cascada automática previa mezclaba precisamente esas responsabilidades en una sola señal de estado. `Used` como estado intrínseco de Evidence Item confundía madurez documental con selección estratégica, que en realidad es una relación específica de cada versión de Blueprint, no un atributo permanente del Evidence Item.

**Alcance:** esta decisión aplica exclusivamente a staleness/reassessment/invalidación disparada por creación, actualización, suplementación, procesamiento, o verificación de un Evidence Item. No supersede la staleness disparada por cambio material de Beneficiary/Petitioner, ni la regla que permite marcar un Generated Document como basado en estrategia desactualizada cuando su Case Blueprint queda Superseded — ambas de disparador arquitectónicamente distinto, preservadas sin cambio.

**Impacto sobre implementaciones existentes:** ningún camino de código en producción implementa hoy el lifecycle lineal, la cascada automática de `stale`, ni Evidence Item como entidad tipada — confirmado en la reconciliación previa (cero ocurrencias de `stale` en `src/`). Esta decisión no requiere ningún cambio inmediato de código, base de datos, ni migraciones. Rige la evolución futura de `AUCIS_EVIDENCE_ITEM_CONTRACT_V1.md` (→ v2), `AUCIS_DOMAIN_INTERACTION_ARCHITECTURE.md`, `AUCIS_CRITERION_ASSESSMENT_CONTRACT_V1.md` (→ v2), `AUCIS_WORKFLOW_STATE_MACHINE_CONTRACT_V1.md` (→ v2), y la entrada Evidence Item de `AUCIS_CORE_DOMAIN_MODEL.md` (ya reconciliada en una sesión previa, pendiente de citar este ADR). El contrato de gobernanza vigente de Case Blueprint (`governance/level-4-contracts/blueprint-contract.md`, Level 4, Frozen) fue verificado como compatible con esta decisión — no requiere modificación. Su especificación delegada (`A5_CASE_BLUEPRINT_SPECIFICATION_V2.md`) es sustancialmente compatible, pero conserva una reconciliación focal pendiente sobre la frase, ya identificada previamente, que describe `criterion_assessment_version_id` como la dependencia que "habilita la cascada de invalidación" — no corregida en esta operación.

**Consecuencias:** Evidence Item pasa a modelarse con Documentary Condition (`Reported | Partial | Documented`) y Human Verification Condition (`Pending | Verified | Needs Attention`) como dimensiones independientes, no secuenciales. `Used` deja de existir como estado — la selección estratégica se representa como relación entre una versión específica de Case Blueprint y el Evidence Item. Evidence puede originarse en cualquier momento del Case, no solo durante Intake, sin que su creación, actualización, suplementación, procesamiento, o verificación dispare automáticamente ni reassessment de A1 ni regeneración de Blueprint — cuando reassessment o regeneración se solicitan específicamente como consecuencia de un cambio en Evidence, requieren la acción humana autorizada correspondiente (esta regla gobierna exclusivamente el efecto de Evidence; no redefine otros workflows/orquestaciones de A1 o A5 fuera de este alcance), preservando los dos caminos ya legítimos (Path A: Evidence sin reassessment; Path B: Evidence con reassessment explícito y nueva versión de Criterion Assessment). El mecanismo físico de versionado/vigencia de Evidence Item queda diferido a la reconciliación de implementación, que deberá evaluar primero la reutilización de patrones ya implementados y probados (`status`/`currency_status`) antes de introducir un mecanismo paralelo.
